...
module.exports = function(io, rfid, arduino1, arduino2, scenario, eventEmitter, logger) {
    ...
    function treat_message(msg) {
        ...
        // Only on the step named "step-1", where students have to sort objects with/without code
        // CodePasCode RFID should send <CODE:12345/> or <PASCODE:12345/> 
        if(scenario.getCurrentStep().stepId == "step-1") {
            if (msg.key ==  "CODE") {
    ...
    function solutionAnalyzisCodePasCode() {
        ...
        if(givenResponseCode !== undefined && givenResponsePasCode != undefined && soluceCode !== undefined && solucePasCode !== undefined) {
            var found = true;
