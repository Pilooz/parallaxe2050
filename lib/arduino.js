...
module.exports = function(arduino, logger, eventEmitter){
    ...
    var _name = ""; // Unique name of arduino provided by himself (message)

    var arduinoReady = false;
    var currentMessage = { key: "", val: "", portNb: _portNb, name: _name };
	...
	if (parseMessage(msg)) {
          ...
          if (currentMessage.key == "NAME") {
            _name = currentMessage.val;
            logger.info(`---------- Arduino '${_name}' is on port '${arduino.portName}' ----------`);
          }
          // Spread over the world that we have an incomming message !
          logger.info(`Emitting a new event 'newArduinoMsg'.`);
          currentMessage.portNb = _portNb;
          currentMessage.name = _name;
    ...
    function setMessageToBlank() {
      currentMessage = { key: "", val: "", portNb: _portNb, name: "" };
    }
    ...
    function sendMessage(k, v){
      ...
      logger.info(`arduino${_portNb[0]} :: Sending ${k}:${v}`);
