...
module.exports = function(global_config, io, scenario, eventEmitter, logger) {
    ...
    var _currentColorsSet = _colorsSet1;
    ...
    lightsIO.on('connection', function(sock) {
        sock.emit("toclient.setColors", { gameMode: _gameMode, set: _currentColorsSet } );
    });
	...
	_currentColorsSet = chooseColorsSet(data.set);
	
        var bigTableau = [];
        var canal = 1;
        _currentColorsSet.forEach(function(c) { bigTableau.push(c.rgb) });
 	...
 	// Send to all browsers that they have to change color.  
        logger.info(`Sending colorsSet #${data.set} to clients.`);
	lightsIO.emit("toclient.setColors", { gameMode: _gameMode, set: _currentColorsSet } );
