...
module.exports = function(global_config, io, scenario, eventEmitter, logger) {
    ...
    const colorsSet1 = DATA_LIGHTS.colorsSet[0].list;
    const colorsSet2 = DATA_LIGHTS.colorsSet[1].list;

    logger.info(`${DATA_LIGHTS.colorsSet.length} sets of lights definitions`);
    logger.info(`${colorsSet1.length} colors in colorSet1.`);
    logger.info(`${colorsSet2.length} colors in colorSet2.`);
    ...
    function chooseColorsSet(set) {
        return ( set == 1) ?  colorsSet1 : colorsSet2; 
    }

    function setBlue() {
        console.log("set Blue");
        artnet.set(1, [0, 0, 255], function (err, res) {
            console.log(err);
            artnet.close();
        });
    }
    ...
    function setGreen() {
        ...
        artnet.set(1, [0, 255, 0], function (err, res) {
            console.log(err);
            artnet.close();
        });
    }
    ...
    function setRed() {
        console.log("set Red");
        artnet.set(1, [255, 0, 0], function (err, res) {
            console.log(err);
            artnet.close();
        });
    }
    ...
    eventEmitter.on('lights.setColors', function(data){
        ...
        artnet.set(1, bigTableau, function (err, res) {
            ...
            if (err) logger.error(`Artnet Error : ${err}`);
            artnet.close();
        });
