...
module.exports = function(io, rfid, arduino1, arduino2, scenario, eventEmitter, logger) {
    ...
    io.on('connection', function(socket) {
        ...
        // Piloter la manivelle depuis le client web
        ...
        // Piloter le téléphone depuis le client web
        socket.on('toserver.telephone_off', function(){
            if (_arduinoTelephone && _arduinoTelephone.isArduinoReady()) {
                _arduinoTelephone.sendMessage("CMD", "STOP");
            }
        });
        ...
        socket.on('toserver.nextStep', function(data) {
            ...
            if (message_telephonique != "" && _arduinoTelephone) {
                ...
                _arduinoTelephone.sendMessage("CMD", "STOP");
                logger.info("Stop telephone for this step.");
            }

            // Pour (presque) faire fermer sa gueule a Marguerite quand ion est sur le knooter
            //  Forcer l'arrêt de la sonnerie du telephone.
            if((["step-2", "step-4", "step-6"].includes(scenario.getCurrentStep().stepId))) {
                _arduinoTelephone.sendMessage("CMD", "STOP");
            }
