...
module.exports = function(io, rfid, arduino1, arduino2, scenario, eventEmitter, logger) {
    ...
    function treat_message(msg) {
        ...
        if ( msg.key == "MSG" && msg.val == "KO" ) {
            logger.error('Something went wrong. See error above.');
            io.emit('toclient.responseAnalysis', {result: "KO", ip: soluceServeurAllie.serveurAllie.ip, errorMessage: "Arduino port is not opened !"});
        }

        // Response to <CMD:CONNECTIONS/> message
        if (msg.key == "CONNECTIONS") {
            var status = ((analizeSolution(msg.val)) ? "OK" : "KO");
            io.emit('toclient.responseAnalysis', {result: status, ip: soluceServeurAllie.serveurAllie.ip, errorMessage: "Réponse fausse."});
        }
    }
