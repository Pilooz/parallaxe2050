 ...
 * 
 */
module.exports = function(io, rfid, arduino, eventArduinoMsg) {
    ...
    io.on('connection', function(socket) {
        ...
        socket.on('toserver.responseAnalysis', function(data) {    
            ...
            arduino.sendMessage("CMD","CONNECTIONS");
        });
    });
    ...
    function analizeSolution(val) {
        ...
        console.log(`servAllie=${servAllie}`);
        console.log(`autreAllie=${autreAllie}`);
        ...
        console.log(`Réponses attendues : ${goodResponses}`);
        console.log(`noeudsATester : ${noeudsATester}`);
        ...
        noeudsATester.forEach(noeud => {
            ...
            if (!goodResponses.includes(noeud)) {
                console.log(`Le noeud ${noeud} n'e devrait pas présent.`);
                soluceTrouvee = false;
            }
        });
        ...
        goodResponses.forEach(rep => {
            if (!noeudsATester.includes(rep)) {
                console.log("Le noeud "+ rep + " était attendu dans les réponses.");
                soluceTrouvee = false;
            }            
        });
        console.log(`soluceTrouvee ? ${soluceTrouvee}`);

        // Décrocher les câbles si c'est faux
        if (!soluceTrouvee) arduino.sendMessage("CMD","RELEASE");
