...
module.exports = function(io, rfid, arduino, scenario, eventEmitter, logger) {
    ...
    function analizeSolution(val) {
        ...
        logger.debug(`servAllie=${servAllie}`);
        logger.debug(`autreAllie=${autreAllie}`);
        ...
        responses.forEach((noeud, index) => {
            logger.debug('--------------------------------');
            logger.debug(`${index} : noeud=${noeud}`);
            if (!found) {
                if (noeud == servAllie ) {
                    if (index == 0) {
        ...
        logger.debug(`Réponses attendues : ${goodResponses}`);
        logger.debug(`noeudsATester : ${noeudsATester}`);
        ...
        noeudsATester.forEach(noeud => {
            ...
            if (!goodResponses.includes(noeud)) {
                logger.debug(`Le noeud ${noeud} n'e devrait pas présent.`);
                soluceTrouvee = false;
            }
        });
        ...
        goodResponses.forEach(rep => {
            if (!noeudsATester.includes(rep)) {
                logger.debug("Le noeud "+ rep + " était attendu dans les réponses.");
                soluceTrouvee = false;
            }            
        });
        logger.debug(`soluceTrouvee ? ${soluceTrouvee}`);
