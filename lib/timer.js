...
module.exports = function(global_config, io, eventEmitter, logger) {
    ...
    //
    function startCount() {
        // TODO : Si le paramètre "LAST_TIME_SAVED" == true dans le fichier data/last-time-saved.json,
        if(LAST_TIME_SAVED.isSaved) {
            _totalTime = LAST_TIME_SAVED.totalTime;
            _timeElapsed = LAST_TIME_SAVED.timeElapsed;
    ...
    function saveTimer() {
        var json = JSON.stringify(_lastTimeSaved);
        logger.info("SAVE TIMER");
        logger.info(json);
        var fs = require('fs');
        fs.writeFile('../data/last-time-saved.json', json, 'utf8');
    }
...
timerIO.on('connection', function(sock){
    ...
    // Controlling timer
    // 
    sock.on('toserver.killTimer', function(data) {
        logger.info(`On vient de killer le serveur '${data.serverId}'`);
        if(data.serverId == "Hardware") {
