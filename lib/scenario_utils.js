...
module.exports = function(config, eventEmitter) {
    ...
    function setCurrentStepId(id) {
        ...
        // If this is the last step, then send another event
        if ( DATA_SCENARIO.steps[DATA_SCENARIO.steps.length - 1].stepId == stepId ) {
          console.log(`'${stepId}' is the last step of ${scenarioId}`);
          eventEmitter.emit('monitoring.lastGameStep', { stepId: stepId });
        }
    ...
    function setSolutionsSetForCurrentStep(group, subgroup) {
        ...
        // Admin badge
        if (group+subgroup == "**") {
            console.info("---- ADMIN BADGE ----");
            console.info("---- Get solution by scenarioId ("+scenarioId+").");
            setObj = DATA_SOLUTIONS_SETS.solutionSets.filter(soluce => (soluce.scenarioId == scenarioId));
        } else {
            console.info(`---- Get solution by group/subgroup (${group}${subgroup}).`);
            setObj = DATA_SOLUTIONS_SETS.solutionSets.filter(soluce => (soluce.team == group + subgroup && soluce.scenarioId == scenarioId));
        }
        // console.log(setObj);
